...
def generate(real_answers:np.array,
    ...
    rho_index = 0
    if show_prgress:
        progress_bar = tqdm(total=max(rho))
    while True:
        # update FEM parameter for the current epsilon
        if rho[rho_index] < cumulative_rho:
            rho_index += 1
            if rho_index == len(rho): break

        rho_0 = rho[rho_index] / rho_split
        epsilon_0 = util2.from_rho_to_eps(rho_0)
        exponential_scale = rho[rho_index] * noise_multiple
        cumulative_rho += rho_0

        if show_prgress:
            progress_bar.update(rho_0)
    ...
    def fem_data_fun(final_rho):
        fem_data = []
        for this_rho, syndata_row in rho_synrow:
            if this_rho > final_rho: break
